...
int
BPlusTree::range(const std::string &start,
...
int
BPlusTree::range_test(const std::string &start, long n) const noexcept
{
    auto leaf = find_leaf(start);
    unsigned long long i = 0;
...
int
BPlusTree::destroy(pmem::obj::pool_base &pop)
{
    if (!root) {
        printf("empty tree\n");
        return 1;
...
int
BPlusTree::insert(pmem::obj::pool_base &pop,
...
void
BPlusTree::print() const noexcept
{
    if (!root) {
        printf("empty tree\n");
        return;
...
void
BPlusTree::print_leaves() const noexcept
{
    auto p = head->next;
    while(p != head) {
